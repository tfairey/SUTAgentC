...
int bDataAllListening = 1;
pthread_t data_threads[1024];

void DataAllStopListening() {
  bDataAllListening = 0;
}
extern void free_key(void *currDir); 

void enddChildListening(void *bListening) {
  puts("BAM!\n");
  *(int*)bListening = 0;
}

void RunDataThread(int sockfd) {
  int nConnections = 0;
  ...
  timeout.tv_usec = 0;
  //Set timeout to 5000?
  pthread_key_create(&dworkerKey, *free_key);
  pthread_key_create(&dlisteningKey, *enddChildListening);
  listen(sockfd, 5);
  while(bDataAllListening) {
    ...
    pthread_create(&data_threads[nConnections], NULL, *runData, clientFD);
    nConnections++;
    nConnections = nConnections % 1024;
  }
  //int i;
  printf("Exiting Data Threads...\n");
  //for(i = 0; i < 1024; i++) {
  //  pthread_join(data_threads[i], NULL); 
  //}
  close(sockfd);
}
/*
